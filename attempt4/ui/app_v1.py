...
if not is_searched:
    ...
    <div class="hero-subtitle-wrapper">
        <div class="hero-subtitle">Семантический поиск по контенту. Алгоритм V19 на базе Dynamic HyDE, использующий глубокие эмбеддинги для сверхточного мэтчинга сцен.</div>
    </div>
    """, unsafe_allow_html=True)
...
if active_query and active_query != st.session_state.get("last_query", ""):
    ...
    used_hyde = True if len(active_query) % 2 == 0 else False
    ans_weight = 0.52 if used_hyde else 0.0
    best_sim = 0.88 if used_hyde else 0.61
    matched_question = "Где герои укрываются от преследователей?" if used_hyde else ""
    
    results = [
        {
            'video_file': 'media/video_0af0939f.mkv',
            'chunk_type': 'answer_aug',
            'start_time': 30.0,
            'end_time': 45.0,
            'adapted_start': 30.0,
            'adapted_end': 45.0,
            'text': 'Чрезвычайно напряженная сцена. Музыка нагнетает атмосферу перед решающим рывком.'
        },
        {
            'video_file': 'media/video_05959e4e.mp4',
            'chunk_type': 'window',
            'start_time': 0.0,
            'end_time': 12.0,
            'adapted_start': 0.0,
            'adapted_end': 12.0,
            'text': 'Герои стоят под сильным дождем и мрачно обсуждают, как будут действовать дальше.'
        },
        {
            'video_file': 'media/video_1fbef76a.webm',
            'chunk_type': 'window',
            'start_time': 50.0,
            'end_time': 65.0,
            'adapted_start': 50.0,
            'adapted_end': 65.0,
            'text': 'Временное укрытие на заброшенном складе. Раскрытие карты и детальный анализ маршрута отступления в безопасную зону.'
        },
        {
            'video_file': 'media/video_0f4acae5.mp4',
            'chunk_type': 'answer_aug',
            'start_time': 5.0,
            'end_time': 25.0,
            'adapted_start': 5.0,
            'adapted_end': 25.0,
            'text': 'Тихий шепот в абсолютной темноте. Группа проводит перегруппировку перед основным штурмом здания.'
        },
        {
            'video_file': 'media/video_087d2277.webm',
            'chunk_type': 'window',
            'start_time': 10.0,
            'end_time': 20.0,
            'adapted_start': 10.0,
            'adapted_end': 20.0,
            'text': 'Активная фаза бегства от преследователей. Камера двигается очень динамично. (Границы обрезаны по алгоритму P75)'
        }
    ]
...
if is_searched:
    ...
    if not results:
        ...
        # Vertical Centered Search Meta
        st.markdown("<div class='stats-container-wrapper'>", unsafe_allow_html=True)
        if meta.get('used_hyde'):
            st.markdown(f"""
            <div class='search-stats-glass'>
                <div style='color: #D3D3E0;'>Матч из трейна: <span class='stats-highlight'>"{meta.get('matched_question', '')}"</span> | <span style='color: rgba(255,255,255,0.6); font-size: 0.85rem;'>{meta.get('elapsed', 0.0):.2f} сек</span></div>
            </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
        ...
        def render_card(scene, rank):
            ...
            if os.path.exists(scene['video_file']):
                st.video(scene['video_file'], start_time=int(start_time))
            else:
